...
const pack   = require('../package.json')
    , parse  = require('./private/parse')
    , concat = require('./private/concat')
...
methods = {
   ...
   */
  get(callback) {
    let header = `/* Generated by Pakket v${pack.version} */\n`;
    header += fs.readFileSync(head, 'utf8').replace('{{bundle:name}}', this.export);
    header += '\n';
    const footer = fs.readFileSync(foot, 'utf8');
    const s = '';
    parse(this.app, s, this.packets, (buf) => {
      buf = concat(header, buf, footer, this.export, this.type);
      callback(buf);
    });
  },
